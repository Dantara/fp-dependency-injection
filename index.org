#+TITLE: Dependency injection
#+SUBTITLE: in functional programming
#+AUTHOR: Nikita Aleshchenko

#+REVEAL_ROOT: https://cdn.jsdelivr.net/npm/reveal.js
#+REVEAL_REVEAL_JS_VERSION: 4
#+REVEAL_THEME: styles/tinkoff.css
#+REVEAL_PLUGINS: (highlight)
#+REVEAL_HIGHLIGHT_CSS: styles/monokai.css
#+REVEAL_EXTRA_CSS: styles/extra.css
#+REVEAL_EXTRA_CSS: https://cdnjs.cloudflare.com/ajax/libs/animate.css/4.1.1/animate.min.css
#+OPTIONS: num:nil toc:nil
#+REVEAL_INIT_OPTIONS: width:1075, navigationMode:"linear"
#+reveal_width: 1075
#+REVEAL_TRANS: slide
#+REVEAL_MARGIN: 0
# #+REVEAL_EXTERNAL_PLUGINS: (Animate . "./plugins/animate.js")

#+REVEAL_TITLE_SLIDE: <img src="images/logo.svg" id="title-img">
#+REVEAL_TITLE_SLIDE: <h2 class="title">%t</h2>
#+REVEAL_TITLE_SLIDE: <h3 class="subtitle">%s</h3>
#+REVEAL_TITLE_SLIDE: <h6 class="author">%a</h6>
#+REVEAL_TITLE_SLIDE_BACKGROUND: rgb(255, 221, 45)
#+REVEAL_TITLE_SLIDE_STATE: title-slide

#+INCLUDE: ./about.html export html

#+INCLUDE: ./qr.html export html

* Мы как разработчики хотим писать...
:PROPERTIES:
:REVEAL_EXTRA_ATTR: data-auto-animate
:END:
* Мы как разработчики хотим писать...
:PROPERTIES:
:REVEAL_EXTRA_ATTR: data-auto-animate
:END:
#+ATTR_REVEAL: :frag appear
+ Поддерживаемый код;
#+ATTR_REVEAL: :frag appear
+ Расширяемый код;
#+ATTR_REVEAL: :frag appear
+ Переиспользуемый код;
#+ATTR_REVEAL: :frag appear
+ Тестируемый код;


* Dependency injection
:PROPERTIES:
:REVEAL_EXTRA_ATTR: data-auto-animate
:END:
* Dependency injection
:PROPERTIES:
:REVEAL_EXTRA_ATTR: data-auto-animate
:END:
Dependency Injection (Внедрение зависимостей) - это метод проектирования программного обеспечения, при котором создание и привязка зависимостей выполняются вне зависимого класса.

** Преимущества DI
#+ATTR_REVEAL: :frag appear
+ Separation of concerns (Разделение ответственностей);
  + Поддерживаемый код;
  + Расширяемый код;
  + Переиспользуемый код;
#+ATTR_REVEAL: :frag appear
+ Тестируемость;

* Onion architecture
:PROPERTIES:
:REVEAL_EXTRA_ATTR: data-auto-animate data-auto-animate-duration="0" data-auto-animate-unmatched="false"
:END:
#+REVEAL_HTML: <span class="first-circle circle"></span>
#+REVEAL_HTML: <span class="second-circle circle"></span>
#+REVEAL_HTML: <span class="third-circle circle"></span>

* Onion architecture
:PROPERTIES:
:REVEAL_EXTRA_ATTR: data-auto-animate data-auto-animate-duration="0" data-auto-animate-unmatched="false"
:END:
#+REVEAL_HTML: <span class="first-circle circle"></span>
#+REVEAL_HTML: <span class="second-circle circle"></span>
#+REVEAL_HTML: <span class="third-circle circle green-border"></span>

* Onion architecture
:PROPERTIES:
:REVEAL_EXTRA_ATTR: data-auto-animate data-auto-animate-duration="0" data-auto-animate-unmatched="false"
:END:
#+REVEAL_HTML: <span class="first-circle circle red-border"></span>
#+REVEAL_HTML: <span class="second-circle circle"></span>
#+REVEAL_HTML: <span class="third-circle circle"></span>

** Трех слойный пирог
| №      | Слой                  |
|--------+-----------------------|
|      3 | Бизнес логика         |
|      2 | DSL для бизнес логики |
|      1 | Интерпретатор         |




* Тестовый проект
** Бот переводчик
:PROPERTIES:
:REVEAL_EXTRA_ATTR: data-auto-animate
:END:
Получает сообщение от пользователя через long polling, переводит с английского на русский, отправляет сообщение обратно пользователю.

** Бот переводчик
:PROPERTIES:
:REVEAL_EXTRA_ATTR: data-auto-animate
:END:
Получает сообщение от пользователя через long polling, переводит с английского на русский, отправляет сообщение обратно пользователю.

+ Получение новых сообщений;
+ Отправка сообщений;
+ Перевод текста;
** Gist
#+ATTR_REVEAL: :code_attribs data-line-numbers='1-23|1-4|6-7|9-10|12-13|15|16|17|19|20|21-32'
#+BEGIN_SRC haskell
data Message = Msg
  { chatId :: Integer
  , text   :: Text
  }

pullUpdates :: IO [Message]
pullUpdates = ...

sendMessage :: Message -> IO ()
sendMessage = ...

translate :: Text -> IO (Maybe Text)
translate = ...

proceed :: IO ()
proceed = pullUpdates
  >>= traverse_ (translateMsg >=> sendMessage)
  where
    translateMsg :: Message -> IO Message
    translateMsg (Msg ci t) = translate t
      <&> (maybe
             (Msg ci "Cannot translate text")
             (Msg ci))
#+END_SRC

** Проблемы
+ Весь код в IO; 🚀
+ Не тестируемый код;


* Handle / Service pattern
** Функциональное программирование
:PROPERTIES:
:REVEAL_EXTRA_ATTR: data-auto-animate
:END:
** Функциональное программирование
:PROPERTIES:
:REVEAL_EXTRA_ATTR: data-auto-animate
:END:
+ Функции;
+ Данные;
** Основная идея
#+ATTR_REVEAL: :data_id handle0 :code_attribs data-trim data-line-numbers
#+BEGIN_SRC haskell
data Handle = Handle
    { hPool   :: Pool Postgres.Connection
    , hCache  :: IORef (PSQueue Int Text User)
    , hLogger :: Logger.Handle  -- Another handle!
    , ...
    }
#+END_SRC
** Handle interface
:PROPERTIES:
:REVEAL_EXTRA_ATTR: data-auto-animate
:END:
#+ATTR_REVEAL: :data_id handle0 :code_attribs data-trim data-line-numbers
#+BEGIN_SRC haskell
data Handle = Handle
    { translate :: Text -> IO (Maybe Text)
    , ...
    }
#+END_SRC

** Handle interface
:PROPERTIES:
:REVEAL_EXTRA_ATTR: data-auto-animate
:END:
#+ATTR_REVEAL: :data_id handle0 :code_attribs data-trim data-line-numbers='1-7|6-7'
#+BEGIN_SRC haskell
data Handle = Handle
    { translate :: Text -> IO (Maybe Text)
    , ...
    }

translate :: Handle -> Text -> IO (Maybe Text)
translate = ...
#+END_SRC

** Наш проект
:PROPERTIES:
:REVEAL_EXTRA_ATTR: data-auto-animate
:END:

#+ATTR_REVEAL: :data_id handle
#+ATTR_REVEAL: :data_id handle :code_attribs data-line-numbers='1-18|1-8' data-trim
#+BEGIN_SRC haskell
pullUpdates :: IO [Message]
pullUpdates = ...

sendMessage :: IO ()
sendMessage = ...

translate :: Text -> IO (Maybe Text)
translate = ...

proceed :: IO ()
proceed = pullUpdates
  >>= traverse_ (translateMsg >=> sendMessage)
  where
    translateMsg :: Message -> IO Message
    translateMsg (Msg ci t) = translate t
      <&> (maybe
             (Msg ci "Cannot translate text")
             (Msg ci))
#+END_SRC

** Наш проект
:PROPERTIES:
:REVEAL_EXTRA_ATTR: data-auto-animate
:END:

#+ATTR_REVEAL: :data_id handle
#+ATTR_REVEAL: :code_attribs data-line-numbers data-trim
#+BEGIN_SRC haskell
pullUpdates :: IO [Message]
pullUpdates = ...

sendMessage :: Message -> IO ()
sendMessage = ...

translate :: Text -> IO (Maybe Text)
translate = ...
#+END_SRC

** Наш проект
:PROPERTIES:
:REVEAL_EXTRA_ATTR: data-auto-animate
:END:

#+ATTR_REVEAL: :data_id handle :code_attribs data-line-numbers
#+BEGIN_SRC haskell
data Handle = Handle
  { pullUpdates :: IO [Message]
  , sendMessage :: Message -> IO ()
  , translate   :: Text -> IO (Maybe Text)
  }
#+END_SRC

** Наш проект
:PROPERTIES:
:REVEAL_EXTRA_ATTR: data-auto-animate
:END:

#+ATTR_REVEAL: :data_id handle :code_attribs data-line-numbers='1-5|1-16|7-16'
#+BEGIN_SRC haskell
data Handle = Handle
  { pullUpdates :: IO [Message]
  , sendMessage :: Message -> IO ()
  , translate   :: Text -> IO (Maybe Text)
  }

proceed :: Handle -> IO ()
proceed h = pullUpdates h
  >>= traverse_ (translateMsg >=> sendMessage h)
  where
    translateMsg :: Message -> m Message
    translateMsg (Msg ci t) = translate h t
      <&> (maybe
             (Msg ci "Cannot translate text")
             (Msg ci))

#+END_SRC

** Параметризуем монаду
:PROPERTIES:
:REVEAL_EXTRA_ATTR: data-auto-animate
:END:

#+ATTR_REVEAL: :data_id handle2 :code_attribs data-line-numbers
#+BEGIN_SRC haskell
data Handle = Handle
  { pullUpdates :: IO [Message]
  , sendMessage :: Message -> IO ()
  , translate   :: Text -> IO (Maybe Text)
  }
#+END_SRC

** Параметризуем монаду
:PROPERTIES:
:REVEAL_EXTRA_ATTR: data-auto-animate
:END:

#+ATTR_REVEAL: :data_id handle2 :code_attribs data-line-numbers
#+BEGIN_SRC haskell
data Handle m = Handle
  { pullUpdates :: m [Message]
  , sendMessage :: Message -> m ()
  , translate   :: Text -> m (Maybe Text)
  }
#+END_SRC

** Наш проект
:PROPERTIES:
:REVEAL_EXTRA_ATTR: data-auto-animate
:END:
#+ATTR_REVEAL: :data_id handle2 :code_attribs data-line-numbers='1-16|7'
#+BEGIN_SRC haskell
data Handle m = Handle
  { pullUpdates :: m [Message]
  , sendMessage :: Message -> m ()
  , translate   :: Text -> m (Maybe Text)
  }

proceed :: MonadIO m => Handle m -> m ()
proceed h = pullUpdates h
  >>= traverse_ (translateMsg >=> sendMessage h)
  where
    translateMsg :: Message -> m Message
    translateMsg (Msg ci t) = translate h t
      <&> (maybe
             (Msg ci "Cannot translate text")
             (Msg ci))

#+END_SRC

** Плюсы & минусы
** Плюсы:
:PROPERTIES:
:REVEAL_EXTRA_ATTR: data-auto-animate
:END:
** Плюсы:
:PROPERTIES:
:REVEAL_EXTRA_ATTR: data-auto-animate
:END:
#+ATTR_REVEAL: :frag appear
  + Просто & Интуитивно;
#+ATTR_REVEAL: :frag appear
  + Производительно;
#+ATTR_REVEAL: :frag appear
  + Подмена хэндлеров в рантайме;

** Минусы:
:PROPERTIES:
:REVEAL_EXTRA_ATTR: data-auto-animate
:END:
** Минусы:
:PROPERTIES:
:REVEAL_EXTRA_ATTR: data-auto-animate
:END:
  + Ручное прокидывание хэндлеров;
  + Привязанность исполняющей монаде;


* Tagless final
:PROPERTIES:
:REVEAL_EXTRA_ATTR: data-auto-animate
:END:
* Tagless final
:PROPERTIES:
:REVEAL_EXTRA_ATTR: data-auto-animate
:END:
Переход от структуры *Handle* к *Type class*'ам!

** Основная идея
:PROPERTIES:
:REVEAL_EXTRA_ATTR: data-auto-animate
:END:
** Основная идея
:PROPERTIES:
:REVEAL_EXTRA_ATTR: data-auto-animate
:END:

#+ATTR_REVEAL: :data_id tf :code_attribs data-line-numbers
#+BEGIN_SRC haskell
data Handle m = Handle
  { pullUpdates :: m [Message]
  , sendMessage :: Message -> m ()
  , translate   :: Text -> m (Maybe Text)
  }
#+END_SRC

** Основная идея
:PROPERTIES:
:REVEAL_EXTRA_ATTR: data-auto-animate
:END:

#+ATTR_REVEAL: :data_id tf :code_attribs data-line-numbers
#+BEGIN_SRC haskell
pullUpdates :: m [Message]
sendMessage :: Message -> m ()
translate   :: Text -> m (Maybe Text)
#+END_SRC

** Основная идея
:PROPERTIES:
:REVEAL_EXTRA_ATTR: data-auto-animate
:END:

#+ATTR_REVEAL: :data_id tf :code_attribs data-line-numbers
#+BEGIN_SRC haskell
pullUpdates :: m [Message]
sendMessage :: Message -> m ()

translate   :: Text -> m (Maybe Text)
#+END_SRC

** Основная идея
:PROPERTIES:
:REVEAL_EXTRA_ATTR: data-auto-animate
:END:

#+ATTR_REVEAL: :data_id tf :code_attribs data-line-numbers
#+BEGIN_SRC haskell
class Monad m => MonadMessenger m where
  pullUpdates :: m [Message]
  sendMessage :: Message -> m ()

class Monad m => MonadTranslator m where
  translate :: Text -> m (Maybe Text)
#+END_SRC

** Основная идея
:PROPERTIES:
:REVEAL_EXTRA_ATTR: data-auto-animate
:END:

Переход от структуры *Handle* к *Type class*'ам!
#+ATTR_REVEAL: :data_id tf :code_attribs data-line-numbers='1-6|1-3|5-6'
#+BEGIN_SRC haskell
class Monad m => MonadMessenger m where
  pullUpdates :: m [Message]
  sendMessage :: Message -> m ()

class Monad m => MonadTranslator m where
  translate :: Text -> m (Maybe Text)
#+END_SRC

** Наш проект
:PROPERTIES:
:REVEAL_EXTRA_ATTR: data-auto-animate
:END:

#+ATTR_REVEAL: :data_id tf :code_attribs data-line-numbers='1-16|8'
#+BEGIN_SRC haskell
class Monad m => MonadMessenger m where
  pullUpdates :: m [Message]
  sendMessage :: Message -> m ()

class Monad m => MonadTranslator m where
  translate :: Text -> m (Maybe Text)

proceed :: (MonadMessenger m, MonadTranslator m) => m ()
proceed = pullUpdates
  >>= traverse_ (translateMsg >=> sendMessage)
  where
    translateMsg :: (MonadTranslator m) => Message -> m Message
    translateMsg (Msg ci t) = translate t
      <&> (maybe
             (Msg ci "Cannot translate text")
             (Msg ci))
#+END_SRC

** Интерпретация
:PROPERTIES:
:REVEAL_EXTRA_ATTR: data-auto-animate
:END:

** Интерпретация
:PROPERTIES:
:REVEAL_EXTRA_ATTR: data-auto-animate
:END:

#+ATTR_REVEAL: :data_id tf-interpret :code_attribs data-trim data-line-numbers
#+BEGIN_SRC haskell
newtype AppM a = AppM { runAppM :: ReaderT SomeContext IO a }
#+END_SRC

** Интерпретация
:PROPERTIES:
:REVEAL_EXTRA_ATTR: data-auto-animate
:END:

#+ATTR_REVEAL: :data_id tf-interpret :code_attribs data-trim data-line-numbers='1-2|2'
#+BEGIN_SRC haskell
newtype AppM a = AppM { runAppM :: ReaderT SomeContext IO a }
  deriving newtype (Functor, Applicative, Monad, ...)
#+END_SRC

** Интерпретация
:PROPERTIES:
:REVEAL_EXTRA_ATTR: data-auto-animate
:END:

#+ATTR_REVEAL: :data_id tf-interpret :code_attribs data-trim data-line-numbers='1-9|4-9'
#+BEGIN_SRC haskell
newtype AppM a = AppM { runAppM :: ReaderT SomeContext IO a }
  deriving newtype (Functor, Applicative, Monad, ...)

instance MonadMessenger AppM where
  pullUpdates = ...
  sendMessage = ...

instance MonadTranslator AppM where
  translate = ...
#+END_SRC

** Интерпретация
:PROPERTIES:
:REVEAL_EXTRA_ATTR: data-auto-animate
:END:

#+ATTR_REVEAL: :data_id tf-interpret :code_attribs data-trim data-line-numbers='1-15|11-15|11|12|13|15|1,15|14'
#+BEGIN_SRC haskell
newtype AppM a = AppM { runAppM :: ReaderT SomeContext IO a }
  deriving newtype (Functor, Applicative, Monad, ...)

instance MonadMessenger AppM where
  pullUpdates = ...
  sendMessage = ...

instance MonadTranslator AppM where
  translate = ...

interpret :: AppM a -> IO a
interpret app = do
  ctx <- retrieveContext
  flip runReaderT ctx
    $ runAppM app
#+END_SRC

** Плюсы & Минусы
** Плюсы
:PROPERTIES:
:REVEAL_EXTRA_ATTR: data-auto-animate
:END:
** Плюсы
:PROPERTIES:
:REVEAL_EXTRA_ATTR: data-auto-animate
:END:
#+ATTR_REVEAL: :frag appear
+ Производительность;
#+ATTR_REVEAL: :frag appear
+ "Автоматическая" подстановка реализации;
#+ATTR_REVEAL: :frag appear
+ Абстракции меньше протекают;
** Минусы
:PROPERTIES:
:REVEAL_EXTRA_ATTR: data-auto-animate
:END:
** Минусы
:PROPERTIES:
:REVEAL_EXTRA_ATTR: data-auto-animate
:END:
+ Часть абстракций протекаeт:
** Минусы
:PROPERTIES:
:REVEAL_EXTRA_ATTR: data-auto-animate
:END:
+ Часть абстракций протекаeт:
#+ATTR_REVEAL: :data_id tf-interpret :code_attribs data-trim
#+BEGIN_SRC haskell
class MonadTranslator m
#+END_SRC
** Минусы
:PROPERTIES:
:REVEAL_EXTRA_ATTR: data-auto-animate
:END:
+ Часть абстракций протекаeт:
#+ATTR_REVEAL: :data_id tf-interpret :code_attribs data-trim
#+BEGIN_SRC haskell
class MonadDB m => MonadTranslator m
#+END_SRC
** Минусы
:PROPERTIES:
:REVEAL_EXTRA_ATTR: data-auto-animate
:END:
+ Часть абстракций протекаeт:
#+ATTR_REVEAL: :data_id tf-interpret :code_attribs data-trim
#+BEGIN_SRC haskell
class MonadTranslator m

instance MonadDB m => MonadTranslator m
#+END_SRC
** Минусы
:PROPERTIES:
:REVEAL_EXTRA_ATTR: data-auto-animate
:END:
+ Нет возможности добавления нового эффекта ниже по стэку:
** Минусы
:PROPERTIES:
:REVEAL_EXTRA_ATTR: data-auto-animate
:END:
+ Нет возможности добавления нового эффекта ниже по стэку:
#+ATTR_REVEAL: :data_id tf-interpret :code_attribs data-trim data-line-numbers
#+BEGIN_SRC haskell
newtype AppM a = AppM { runAppM :: ReaderT SomeContext IO a }
  deriving newtype (Functor, Applicative, Monad, ...)

instance MonadMessenger AppM where
  pullUpdates = ...
  sendMessage = ...

instance MonadTranslator AppM where
  translate = ...
#+END_SRC
** Минусы
:PROPERTIES:
:REVEAL_EXTRA_ATTR: data-auto-animate
:END:
+ Нет возможности добавления нового эффекта ниже по стэку:
#+ATTR_REVEAL: :data_id tf-interpret :code_attribs data-trim data-line-numbers='1-9|11-16|13'
#+BEGIN_SRC haskell
newtype AppM a = AppM { runAppM :: ReaderT SomeContext IO a }
  deriving newtype (Functor, Applicative, Monad, ...)

instance MonadMessenger AppM where
  pullUpdates = ...
  sendMessage = ...

instance MonadTranslator AppM where
  translate = ...

instance MonadMessenger m => MonadMessenger (ReaderT r m) where
  pullUpdates     = ReaderT $ \_ -> pullUpdates
  sendMessage msg = ReaderT $ \_ -> sendMessage msg

instance MonadTranslator m => MonadTranslator (ReaderT r m) where
  translate t = ReaderT $ \_ -> translate t
#+END_SRC
** Минусы
:PROPERTIES:
:REVEAL_EXTRA_ATTR: data-auto-animate
:END:
+ Нет возможности "реинтрепретации" эффекта:
** Минусы
:PROPERTIES:
:REVEAL_EXTRA_ATTR: data-auto-animate
:END:
+ Нет возможности "реинтрепретации" эффекта:
#+ATTR_REVEAL: :data_id tf-reinterpret :code_attribs data-trim data-line-numbers
#+BEGIN_SRC haskell
newtype AppM a = AppM { runAppM :: ReaderT SomeContext IO a }
  deriving newtype (Functor, Applicative, Monad, ...)

instance MonadMessenger AppM where
  pullUpdates = ...
  sendMessage = ...

instance MonadTranslator AppM where
  translate = ...
#+END_SRC

** Минусы
:PROPERTIES:
:REVEAL_EXTRA_ATTR: data-auto-animate
:END:
+ Нет возможности "реинтрепретации" эффекта:
#+ATTR_REVEAL: :data_id tf-reinterpret :code_attribs data-trim data-line-numbers='1-9|11-19'
#+BEGIN_SRC haskell
newtype AppM a = AppM { runAppM :: ReaderT SomeContext IO a }
  deriving newtype (Functor, Applicative, Monad, ...)

instance MonadMessenger AppM where
  pullUpdates = ...
  sendMessage = ...

instance MonadTranslator AppM where
  translate = ...

newtype AppM2 a = AppM2 { runAppM2 :: ReaderT SomeContext IO a }
  deriving newtype (Functor, Applicative, Monad, ...)

instance MonadMessenger AppM2 where
  pullUpdates = ...
  sendMessage = ...

instance MonadTranslator AppM2 where
  translate = ...
#+END_SRC


* Free monads
** Интерпретируемые языки
:PROPERTIES:
:REVEAL_EXTRA_ATTR: data-auto-animate
:END:
** Интерпретируемые языки
:PROPERTIES:
:REVEAL_EXTRA_ATTR: data-auto-animate
:END:

# #+ATTR_HTML: :width 40% :height 40% :style position: absolute left: 10vw;
# [[./images/ruby.png]]

#+ATTR_REVEAL: :frag appear
+ Код;
#+ATTR_REVEAL: :frag appear
+ AST (Abstract syntax tree);
#+ATTR_REVEAL: :frag appear
+ Интрепретатор;

** Большая троица

*Functor*
#+BEGIN_SRC haskell
class Functor f where
  fmap :: (a -> b) -> f a -> f b
#+END_SRC

*Applicative*
#+BEGIN_SRC haskell
class Functor f => Applicative f where
  pure :: a -> f a
  (<*>) :: f (a -> b) -> f a -> f b
#+END_SRC

*Monad*
#+BEGIN_SRC haskell
class Applicative m => Monad m where
  return :: a -> m a
  (>>=) :: m a -> (a -> m b) -> m b
#+END_SRC

** Functor
#+ATTR_REVEAL: :code_attribs data-trim data-line-numbers
#+BEGIN_SRC haskell
class Functor f where
  fmap :: (a -> b) -> f a -> f b
#+END_SRC

** Applicative
#+ATTR_REVEAL: :code_attribs data-trim data-line-numbers
#+BEGIN_SRC haskell
class Functor f => Applicative f where
  pure :: a -> f a
  (<*>) :: f (a -> b) -> f a -> f b
#+END_SRC

** Monad
:PROPERTIES:
:REVEAL_EXTRA_ATTR: data-auto-animate
:END:
#+ATTR_REVEAL: :data_id monad :code_attribs data-trim data-line-numbers
#+BEGIN_SRC haskell
class Applicative m => Monad m where
  return :: a -> m a
  (>>=) :: m a -> (a -> m b) -> m b
#+END_SRC

** Monad
:PROPERTIES:
:REVEAL_EXTRA_ATTR: data-auto-animate
:END:
#+ATTR_REVEAL: :data_id monad :code_attribs data-trim data-line-numbers
#+BEGIN_SRC haskell
class Applicative m => Monad m where
  return :: a -> m a
  (>>=) :: m a -> (a -> m b) -> m b

join :: Monad m => m (m a) -> m a
#+END_SRC

** Monad
:PROPERTIES:
:REVEAL_EXTRA_ATTR: data-auto-animate
:END:
#+ATTR_REVEAL: :data_id monad :code_attribs data-trim data-line-numbers
#+BEGIN_SRC haskell
fmap :: Functor f => (a -> b) -> f a -> f b
join :: Monad m => m (m a) -> m a
#+END_SRC

** Monad
:PROPERTIES:
:REVEAL_EXTRA_ATTR: data-auto-animate
:END:
#+ATTR_REVEAL: :data_id monad :code_attribs data-trim data-line-numbers=1-5|4-5
#+BEGIN_SRC haskell
fmap :: Functor f => (a -> b) -> f a -> f b
join :: Monad m => m (m a) -> m a

(>>=) :: m a -> (a -> m b) -> m b
(>>=) m f = join (fmap f m)
#+END_SRC

** Алгебра бизнес логики
:PROPERTIES:
:REVEAL_EXTRA_ATTR: data-auto-animate
:END:
#+ATTR_REVEAL: :data_id algebra :code_attribs data-trim data-line-numbers
#+BEGIN_SRC haskell
data Bot
  = PullUpdates [Message]
  | SendMessage Message
  | Translate Text (Maybe Text)
#+END_SRC

** Алгебра бизнес логики
:PROPERTIES:
:REVEAL_EXTRA_ATTR: data-auto-animate
:END:

#+ATTR_REVEAL: :data_id algebra :code_attribs data-trim data-line-numbers
#+BEGIN_SRC haskell
data Bot
  = PullUpdate Message
  | SendMessage Message
#+END_SRC

** Алгебра бизнес логики
:PROPERTIES:
:REVEAL_EXTRA_ATTR: data-auto-animate
:END:

#+ATTR_REVEAL: :data_id algebra :code_attribs data-trim data-line-numbers
#+BEGIN_SRC haskell
data Bot next
  = PullUpdate Message next
  | SendMessage Message next
#+END_SRC

** Алгебра бизнес логики
:PROPERTIES:
:REVEAL_EXTRA_ATTR: data-auto-animate
:END:

#+ATTR_REVEAL: :data_id algebra :code_attribs data-trim data-line-numbers
#+BEGIN_SRC haskell
data Bot a
  = PullUpdate Message (Bot a)
  | SendMessage Message (Bot a)
#+END_SRC

** Алгебра бизнес логики
:PROPERTIES:
:REVEAL_EXTRA_ATTR: data-auto-animate
:END:

#+ATTR_REVEAL: :data_id algebra :code_attribs data-trim data-line-numbers
#+BEGIN_SRC haskell
data Bot a
  = PullUpdate Message (Bot a)
  | SendMessage Message (Bot a)
#+END_SRC

#+ATTR_HTML: :height 200vh :style margin-left: 50%; transform: translateX(-50%);
[[./images/dolls.png]]

** Алгебра бизнес логики
:PROPERTIES:
:REVEAL_EXTRA_ATTR: data-auto-animate
:END:

#+ATTR_REVEAL: :data_id algebra :code_attribs data-trim data-line-numbers
#+BEGIN_SRC haskell
data Bot a
  = PullUpdate (Message -> (Bot a))
  | SendMessage Message (Bot a)
#+END_SRC

** Алгебра бизнес логики
:PROPERTIES:
:REVEAL_EXTRA_ATTR: data-auto-animate
:END:

#+ATTR_REVEAL: :data_id algebra :code_attribs data-trim data-line-numbers
#+BEGIN_SRC haskell
data Bot a
  = PullUpdate (Message -> (Bot a))
  | SendMessage Message (Bot a)
  | Done a
#+END_SRC

** Эхо бот
:PROPERTIES:
:REVEAL_EXTRA_ATTR: data-auto-animate
:END:

#+ATTR_REVEAL: :data_id algebra :code_attribs data-trim data-line-numbers=1-9|6-9|6|7|8|9
#+BEGIN_SRC haskell
data Bot a
  = PullUpdate (Message -> (Bot a))
  | SendMessage Message (Bot a)
  | Done a

echo :: Bot ()
echo = PullUpdate $ \msg ->
       SendMessage msg
     $ Done ()
#+END_SRC
** Эхо бот
:PROPERTIES:
:REVEAL_EXTRA_ATTR: data-auto-animate
:END:

#+ATTR_REVEAL: :data_id algebra :code_attribs data-trim data-line-numbers
#+BEGIN_SRC haskell
data Bot a
  = PullUpdate (Message -> (Bot a))
  | SendMessage Message (Bot a)
  | Done a

echo :: Bot ()
echo = PullUpdate $ \msg ->
       SendMessage msg
     $ Done ()
#+END_SRC

#+ATTR_HTML: :height 200vh :style margin-left: 50%; transform: translateX(-50%);
[[./images/dolls.png]]

** Monad instance
:PROPERTIES:
:REVEAL_EXTRA_ATTR: data-auto-animate
:END:

#+ATTR_REVEAL: :data_id algebra :code_attribs data-trim data-line-numbers=1-13|6-8|10-13|11|12|13
#+BEGIN_SRC haskell
data Bot a
  = SendMessage Message (Bot a)
  | PullUpdate (Message -> (Bot a))
  | Done a

class Monad m where
  pure  :: m a
  (>>=) :: m a -> (a -> m b) -> m b

instance Monad Bot where
  pure = Done
  SendMessage msg k >>= f = SendMessage msg $ k >>= f
  PullUpdate      k >>= f = PullUpdate $ \msg -> k msg >>= f
#+END_SRC

** Эхо бот
:PROPERTIES:
:REVEAL_EXTRA_ATTR: data-auto-animate
:END:
#+ATTR_REVEAL: :data_id algebra :code_attribs data-trim data-line-numbers=1-9|6-9
#+BEGIN_SRC haskell
data Bot a
  = PullUpdate (Message -> (Bot a))
  | SendMessage Message (Bot a)
  | Done a

echo :: Bot ()
echo = do
  msg <- PullUpdate Done
  SendMessage msg $ Done ()
#+END_SRC

** Интерпретация
:PROPERTIES:
:REVEAL_EXTRA_ATTR: data-auto-animate
:END:
#+ATTR_REVEAL: :data_id algebra :code_attribs data-trim data-line-numbers=1-13|1-4|6-13|6|7|8-10|11-13
#+BEGIN_SRC haskell
data Bot a
  = Done a
  | SendMessage Message (Bot a)
  | PullUpdate (Message -> (Bot a))

interpretBotIO :: Bot a -> IO a
interpretBotIO (Done a) = pure a
interpretBotIO (SendMessage msg m) = do
  sendMessageIO msg
  interpretBotIO m
interpretBotIO (PullUpdate f) = do
  msg <- pullMessageIO
  interpretBotIO $ f msg
#+END_SRC

** Абстрагирование
:PROPERTIES:
:REVEAL_EXTRA_ATTR: data-auto-animate
:END:
#+ATTR_REVEAL: :data_id algebra :code_attribs data-trim data-line-numbers
#+BEGIN_SRC haskell
data Bot a
  = SendMessage Message (Bot a)
  | PullUpdate (Message -> (Bot a))
  | Done a
#+END_SRC

** Абстрагирование
:PROPERTIES:
:REVEAL_EXTRA_ATTR: data-auto-animate
:END:
#+ATTR_REVEAL: :data_id algebra :code_attribs data-trim data-line-numbers
#+BEGIN_SRC haskell
data Bot a
  = SendMessage Message (Bot a)
  | PullUpdate (Message -> (Bot a))
  | Translate Text ((Maybe Text) -> Bot a)
  | Done a
#+END_SRC

** Абстрагирование
:PROPERTIES:
:REVEAL_EXTRA_ATTR: data-auto-animate
:END:
#+ATTR_REVEAL: :data_id algebra :code_attribs data-trim data-line-numbers=1-10|1-8|10
#+BEGIN_SRC haskell
data BotF next
  = SendMessage Message next
  | PullUpdate (Message -> next)
  | Translate Text ((Maybe Text) -> next)

data Free f a
  = Pure a
  | Impure (f (Free f a))

type Bot a = Free BotF a
#+END_SRC

** Основная идея
:PROPERTIES:
:REVEAL_EXTRA_ATTR: data-auto-animate
:END:
#+ATTR_REVEAL: :data_id free :code_attribs data-trim data-line-numbers
#+BEGIN_SRC haskell
data Free f a
  = Pure a
  | Impure (f (Free f a))
#+END_SRC

** Основная идея
:PROPERTIES:
:REVEAL_EXTRA_ATTR: data-auto-animate
:END:
#+ATTR_REVEAL: :data_id free :code_attribs data-trim data-line-numbers=1-3|5,8|6,9
#+BEGIN_SRC haskell
data Free f a
  = Pure a
  | Impure (f (Free f a))

Pure   :: a -> Free f a             -- looks like pure
Impure :: f (Free f a) -> Free f a  -- looks like join

pure :: Monad m => a -> m a
join :: Monad m => m (m a) -> m a
#+END_SRC

** Functor instance
:PROPERTIES:
:REVEAL_EXTRA_ATTR: data-auto-animate
:END:
#+ATTR_REVEAL: :data_id free :code_attribs data-trim data-line-numbers=1-7|5-7
#+BEGIN_SRC haskell
data Free f a
  = Pure a
  | Impure (f (Free f a))

instance Functor f => Functor (Free f) where
  fmap g (Pure x)    = Pure (g x)
  fmap g (Impure fx) = Impure (fmap g <$> fx)
#+END_SRC

** Monad instance
:PROPERTIES:
:REVEAL_EXTRA_ATTR: data-auto-animate
:END:
#+ATTR_REVEAL: :data_id free :code_attribs data-trim data-line-numbers=1-8|5-8
#+BEGIN_SRC haskell
data Free f a
  = Pure a
  | Impure (f (Free f a))

instance Functor f => Monad (Free f) where
  pure = Pure
  Pure x    >>= g = g x
  Impure fx >>= g = Impure ((>>= g) <$> fx)
#+END_SRC

** Алгебра бизнес логики
:PROPERTIES:
:REVEAL_EXTRA_ATTR: data-auto-animate
:END:

#+ATTR_REVEAL: :data_id free :code_attribs data-trim data-line-numbers
#+BEGIN_SRC haskell
data BotF a
  = PullUpdates ([Message] -> a)
  | SendMessage Message a
  | Translate Text ((Maybe Text) -> a)
  deriving (Functor)

type Bot = Free BotF
#+END_SRC

** Lift functor
:PROPERTIES:
:REVEAL_EXTRA_ATTR: data-auto-animate
:END:
#+ATTR_REVEAL: :data_id free :code_attribs data-trim data-line-numbers
#+BEGIN_SRC haskell
liftF :: Functor f => f a -> Free f a
liftF command = Free (fmap Pure command)
#+END_SRC

** Хелперы
:PROPERTIES:
:REVEAL_EXTRA_ATTR: data-auto-animate
:END:
#+ATTR_REVEAL: :data_id free :code_attribs data-trim data-line-numbers
#+BEGIN_SRC haskell
data BotF a
  = PullUpdates ([Message] -> a)
  | SendMessage Message a
  | Translate Text ((Maybe Text) -> a)

pullUpdates :: Bot [Message]
pullUpdates = liftF (PullUpdates id)

sendMessage :: Message -> Bot ()
sendMessage msg = liftF (SendMessage msg ())

translate :: Text -> Bot (Maybe Text)
translate t = liftF (Translate t id)
#+END_SRC

** Бизнес логика
#+BEGIN_SRC haskell
proceed :: Bot ()
proceed = pullUpdates
  >>= traverse_ (translateMsg >=> sendMessage)
  where
    translateMsg :: Message -> Bot Message
    translateMsg (Msg ci t) = translate t
      <&> (maybe
             (Msg ci "Cannot translate text")
             (Msg ci))
#+END_SRC

** Интерпретация
:PROPERTIES:
:REVEAL_EXTRA_ATTR: data-auto-animate
:END:
#+ATTR_REVEAL: :data_id free :code_attribs data-trim data-line-numbers=1-14|6-9|11-14
#+BEGIN_SRC haskell
data BotF a
  = PullUpdates ([Message] -> a)
  | SendMessage Message a
  | Translate Text ((Maybe Text) -> a)

botIO :: BotF a -> IO a
botIO (PullUpdates u) = ...
botIO (SendMessage m a) = ...
botIO (Translate t ft) = ...

type Bot = Free BotF

interpretBotIO :: Bot a -> IO a
interpretBotIO = ???
#+END_SRC

** Хелперы
:PROPERTIES:
:REVEAL_EXTRA_ATTR: data-auto-animate
:END:

#+ATTR_REVEAL: :data_id free-helpers :code_attribs data-trim data-line-numbers=1-7|2-3|4|5|6|7
#+BEGIN_SRC haskell
freeM
  :: (Functor f, Functor g)
  => (f a -> g a)
  -> Free f a
  -> Free g a
freeM phi (Pure x)    = Pure x
freeM phi (Impure fx) = Impure $ phi (freeM phi <$> fx)
#+END_SRC

** Хелперы
:PROPERTIES:
:REVEAL_EXTRA_ATTR: data-auto-animate
:END:
#+ATTR_REVEAL: :data_id free-helpers :code_attribs data-trim data-line-numbers=9-13|9
#+BEGIN_SRC haskell
freeM
  :: (Functor f, Functor g)
  => (f a -> g a)
  -> Free f a
  -> Free g a
freeM phi (Pure x)    = Pure x
freeM phi (Impure fx) = Impure $ phi (freeM phi <$> fx)

monad :: Monad m => Free m a -> m a
monad (Pure x) = pure x
monad (Free mfx) = do
  fx <- mfx
  monad fx
#+END_SRC

** Хелперы
:PROPERTIES:
:REVEAL_EXTRA_ATTR: data-auto-animate
:END:
#+ATTR_REVEAL: :data_id free-helpers :code_attribs data-trim data-line-numbers=1-20|15-20
#+BEGIN_SRC haskell
freeM
  :: (Functor f, Functor g)
  => (f a -> g a)
  -> Free f a
  -> Free g a
freeM phi (Pure x)    = Pure x
freeM phi (Impure fx) = Impure $ phi (freeM phi <$> fx)

monad :: Monad m => Free m a -> m a
monad (Pure x) = pure x
monad (Free mfx) = do
  fx <- mfx
  monad fx

interpret
  :: (Functor f, Monad m)
  => (f a -> m a)
  -> Free f a
  -> m a
interpret phi = monad . freeM phi
#+END_SRC

** Интерпретация
:PROPERTIES:
:REVEAL_EXTRA_ATTR: data-auto-animate
:END:
#+ATTR_REVEAL: :data_id free-helpers :code_attribs data-trim data-line-numbers=1-10|1|3-7|9-10
#+BEGIN_SRC haskell
botIO :: BotF a -> IO a

interpret
  :: (Functor f, Monad m)
  => (f a -> m a)
  -> Free f a
  -> m a

interpetBotIO :: Bot a -> IO a
interpetBotIO = interpret botIO
#+END_SRC

** Композиция алгебр
:PROPERTIES:
:REVEAL_EXTRA_ATTR: data-auto-animate
:END:
#+ATTR_REVEAL: :data_id free-comp :code_attribs data-trim data-line-numbers
#+BEGIN_SRC haskell
data BotF a
  = PullUpdates ([Message] -> a)
  | SendMessage Message a
  | Translate Text ((Maybe Text) -> a)

type Bot = Free BotF
#+END_SRC

** Композиция алгебр
:PROPERTIES:
:REVEAL_EXTRA_ATTR: data-auto-animate
:END:
#+ATTR_REVEAL: :data_id free-comp :code_attribs data-trim data-line-numbers
#+BEGIN_SRC haskell
data MessengerF a
  = PullUpdates ([Message] -> a)
  | SendMessage Message a

data TranslatorF a
  = Translate Text ((Maybe Text) -> a)
#+END_SRC

** Тип суммы
:PROPERTIES:
:REVEAL_EXTRA_ATTR: data-auto-animate
:END:
#+ATTR_REVEAL: :data_id free-comp :code_attribs data-trim data-line-numbers
#+BEGIN_SRC haskell
data Sum f g a
  = L (f a)
  | R (g a)

instance (Functor f, Functor g) => Functor (Sum f g)
#+END_SRC

** Композиция алгебр
:PROPERTIES:
:REVEAL_EXTRA_ATTR: data-auto-animate
:END:
#+ATTR_REVEAL: :data_id free-comp :code_attribs data-trim data-line-numbers=1-10|10
#+BEGIN_SRC haskell
data MessengerF a
  = PullUpdates ([Message] -> a)
  | SendMessage Message a
  deriving (Functor)

data TranslatorF a
  = Translate Text ((Maybe Text) -> a)
  deriving (Functor)

type BotF = Sum MessengerF TranslatorF
#+END_SRC

** Хелперы
:PROPERTIES:
:REVEAL_EXTRA_ATTR: data-auto-animate
:END:
#+ATTR_REVEAL: :data_id free-comp :code_attribs data-trim data-line-numbers
#+BEGIN_SRC haskell
pullUpdates :: Bot [Message]
pullUpdates = liftF $ PullUpdates id

sendMessage :: Message -> Bot ()
sendMessage msg = liftF $ SendMessage msg ()

translate :: Text -> Bot (Maybe Text)
translate t = liftF $ Translate t id
#+END_SRC

** Хелперы
:PROPERTIES:
:REVEAL_EXTRA_ATTR: data-auto-animate
:END:
#+ATTR_REVEAL: :data_id free-comp :code_attribs data-trim data-line-numbers
#+BEGIN_SRC haskell
pullUpdates :: Bot [Message]
pullUpdates = liftF $ L $ PullUpdates id

sendMessage :: Message -> Bot ()
sendMessage msg = liftF $ L $ SendMessage msg ()

translate :: Text -> Bot (Maybe Text)
translate t = liftF $ R $ Translate t id
#+END_SRC

** Тип суммы
#+ATTR_REVEAL: :code_attribs data-trim data-line-numbers
#+BEGIN_SRC haskell
data Sum f g a
  = L (f a)
  | R (g a)
#+END_SRC

** Варианты расширения
:PROPERTIES:
:REVEAL_EXTRA_ATTR: data-auto-animate
:END:
** Варианты расширения
:PROPERTIES:
:REVEAL_EXTRA_ATTR: data-auto-animate
:END:
+ Постороение дерева (Hierarchical Free Monads);
+ Расширяемый спиcок эффектов (Extensible effects);

** Hierarchical Free Monads
:PROPERTIES:
:REVEAL_EXTRA_ATTR: data-auto-animate
:END:
#+ATTR_REVEAL: :data_id hfm :code_attribs data-trim data-line-numbers
#+BEGIN_SRC haskell
data MessengerF next
  = PullUpdates ([Message] -> next)
  | SendMessage Message next

data TranslatorF next
  = Translate Text ((Maybe Text) -> next)
#+END_SRC

** Hierarchical Free Monads
:PROPERTIES:
:REVEAL_EXTRA_ATTR: data-auto-animate
:END:
#+ATTR_REVEAL: :data_id hfm :code_attribs data-trim data-line-numbers=1-10|8-10
#+BEGIN_SRC haskell
data MessengerF next
  = PullUpdates ([Message] -> next)
  | SendMessage Message next

data TranslatorF next
  = Translate Text ((Maybe Text) -> next)

data BotF a
  = EvalMessenger  (MessengerF a)
  | EvalTranslator (TranslatorF a)
#+END_SRC

** Hierarchical Free Monads
:PROPERTIES:
:REVEAL_EXTRA_ATTR: data-auto-animate
:END:
#+ATTR_REVEAL: :data_id hfm :code_attribs data-trim data-line-numbers
#+BEGIN_SRC haskell
data BotF a
  = EvalMessenger  (MessengerF a)
  | EvalTranslator (TranslatorF a)
  | EvalLogger     (LoggerF a)
#+END_SRC

** Hierarchical Free Monads
:PROPERTIES:
:REVEAL_EXTRA_ATTR: data-auto-animate
:END:
#+ATTR_REVEAL: :data_id hfm :code_attribs data-trim data-line-numbers
#+BEGIN_SRC haskell
data BotF a
  = EvalMessenger  (MessengerF a)
  | EvalTranslator (TranslatorF a)
  | EvalLogger     (LoggerF a)

botIO :: BotF a -> IO a
botIO (EvalMessenger m)  = messengerIO m
botIO (EvalTranslator m) = translatorIO m
botIO (EvalLogger m)     = evalLoggerIO m
#+END_SRC


** Плюсы & Минусы

** Плюсы:
:PROPERTIES:
:REVEAL_EXTRA_ATTR: data-auto-animate
:END:
** Плюсы:
:PROPERTIES:
:REVEAL_EXTRA_ATTR: data-auto-animate
:END:
+ Высокая выразительность;
+ Чистота бизнес логики;
+ Жеское разделение слоев (при HFM);

** Минусы:
:PROPERTIES:
:REVEAL_EXTRA_ATTR: data-auto-animate
:END:
** Минусы:
:PROPERTIES:
:REVEAL_EXTRA_ATTR: data-auto-animate
:END:
+ Производительность (при initial кодировке);
+ Boilerplate;
+ Эффекты высшего порядка;

** Эффекты высшего порядка

** Функции высшего порядка
:PROPERTIES:
:REVEAL_EXTRA_ATTR: data-auto-animate
:END:
#+ATTR_REVEAL: :code_attribs data-trim data-line-numbers
#+BEGIN_SRC haskell
map :: (a -> b) -> [a] -> [b]

filter :: (a -> Bool) -> [a] -> [a]
#+END_SRC

** Эффекты высшего порядка
:PROPERTIES:
:REVEAL_EXTRA_ATTR: data-auto-animate
:END:
#+ATTR_REVEAL: :data_id hfm :code_attribs data-trim data-line-numbers=1-3|3
#+BEGIN_SRC haskell
class MonadReader r m | m -> r where
  ask   :: m r
  local :: (r -> r) -> m a -> ma
#+END_SRC

** Эффекты высшего порядка
:PROPERTIES:
:REVEAL_EXTRA_ATTR: data-auto-animate
:END:
#+ATTR_REVEAL: :data_id hfm :code_attribs data-trim data-line-numbers=1-3|3
#+BEGIN_SRC haskell
class MonadError e m | m -> e where
  throwError :: e -> m a
  catchError :: m a -> (e -> m a) -> m a
#+END_SRC

* Freer monads
** Generilized Algebraic Data Types (GADT)
:PROPERTIES:
:REVEAL_EXTRA_ATTR: data-auto-animate
:END:
** Generilized Algebraic Data Types (GADT)
:PROPERTIES:
:REVEAL_EXTRA_ATTR: data-auto-animate
:END:

#+ATTR_REVEAL: :data_id gadt :code_attribs data-line-numbers
#+BEGIN_SRC haskell
data SomeType a
  = First a
  | Second a
#+END_SRC

** Generilized Algebraic Data Types (GADT)
:PROPERTIES:
:REVEAL_EXTRA_ATTR: data-auto-animate
:END:

#+ATTR_REVEAL: :data_id gadt :code_attribs data-line-numbers
#+BEGIN_SRC haskell
data SomeType a where
  First  :: a -> SomeType a
  Second :: a -> SomeType a
#+END_SRC

** Generilized Algebraic Data Types (GADT)
:PROPERTIES:
:REVEAL_EXTRA_ATTR: data-auto-animate
:END:

#+ATTR_REVEAL: :data_id gadt :code_attribs data-line-numbers
#+BEGIN_SRC haskell
data SomeType a where
  First  :: a -> SomeType a
  Second :: Int -> SomeType Int
#+END_SRC

** Generilized Algebraic Data Types (GADT)
:PROPERTIES:
:REVEAL_EXTRA_ATTR: data-auto-animate
:END:

#+ATTR_REVEAL: :data_id gadt :code_attribs data-line-numbers
#+BEGIN_SRC haskell
data SomeType a where
  First  :: b -> SomeType b
  Second :: Int -> SomeType Int
#+END_SRC

** Free monad
:PROPERTIES:
:REVEAL_EXTRA_ATTR: data-auto-animate
:END:

#+ATTR_REVEAL: :data_id freer :code_attribs data-line-numbers
#+BEGIN_SRC haskell
data Free f a
  = Pure a
  | Impure (f (Free f a))
#+END_SRC
** Free monad
:PROPERTIES:
:REVEAL_EXTRA_ATTR: data-auto-animate
:END:

#+ATTR_REVEAL: :data_id freer :code_attribs data-line-numbers
#+BEGIN_SRC haskell
data Free f a where
  Pure   :: a -> Free f a
  Impure :: f (Free f a) -> Free f a
#+END_SRC

** Простой трюк
:PROPERTIES:
:REVEAL_EXTRA_ATTR: data-auto-animate
:END:
#+ATTR_REVEAL: :data_id freer :code_attribs data-line-numbers=1-2|4-5|7-8
#+BEGIN_SRC haskell
data Lan g a where
  Lan :: g x -> (x -> a) -> Lan g a

instance Functor (Lan g) where
  fmap f (Lan gx h) = Lan gx (f . h)

lan :: g a -> Lan g a
lan ga = Lan ga id
#+END_SRC

** Простой трюк
:PROPERTIES:
:REVEAL_EXTRA_ATTR: data-auto-animate
:END:
#+ATTR_REVEAL: :data_id freer :code_attribs data-line-numbers=1-5|2,5
#+BEGIN_SRC haskell
data Lan g b where
  Lan :: (a -> b) -> g a -> Lan g b

class Functor f where
  fmap :: (a -> b) -> f a -> f b
#+END_SRC

** Простой трюк
:PROPERTIES:
:REVEAL_EXTRA_ATTR: data-auto-animate
:END:
#+ATTR_REVEAL: :data_id freer :code_attribs data-line-numbers data-trim data-line-numbers='1-14|1-5|8-14'
#+BEGIN_SRC haskell
data Lan g b where
  Lan :: (a -> b) -> g a -> Lan g b

class Functor f where
  fmap :: (a -> b) -> f a -> f b


data FFree g a where
  FPure   :: a -> FFree g a
  FImpure :: g x -> (x -> FFree g a) -> FFree g a

class Monad m where
  pure  :: a -> m a
  (>>=) :: m a -> (a -> m b) -> m b
#+END_SRC

** Простой трюк
:PROPERTIES:
:REVEAL_EXTRA_ATTR: data-auto-animate
:END:
#+ATTR_REVEAL: :data_id freer :code_attribs data-line-numbers
#+BEGIN_SRC haskell
data FFree g a where
  FPure   :: a -> FFree g a
  FImpure :: g x -> (x -> FFree g a) -> FFree g a

class Monad m where
  pure  :: a -> m a
  (>>=) :: m a -> (a -> m b) -> m b
#+END_SRC

** Простой трюк
:PROPERTIES:
:REVEAL_EXTRA_ATTR: data-auto-animate
:END:
#+ATTR_REVEAL: :data_id freer :code_attribs data-line-numbers=1-9|1
#+BEGIN_SRC haskell
FFree g = Free (Lan g)

data FFree g a where
  FPure   :: a -> FFree g a
  FImpure :: g x -> (x -> FFree g a) -> FFree g a

class Monad m where
  pure  :: a -> m a
  (>>=) :: m a -> (a -> m b) -> m b
#+END_SRC


** Алгебра бизнес логики
#+ATTR_REVEAL: :data_id freer :code_attribs data-line-numbers
#+BEGIN_SRC haskell
data Bot a where
  PullUpdates :: Bot [Message]
  SendMessage :: Message -> Bot ()
  Translate   :: Text -> Bot (Maybe Text)

type BotM = FFree Bot
#+END_SRC

** Lift helper
:PROPERTIES:
:REVEAL_EXTRA_ATTR: data-auto-animate
:END:
#+ATTR_REVEAL: :data_id freer :code_attribs data-line-numbers
#+BEGIN_SRC haskell
lan :: g a -> Lan g a
lan ga = Lan ga id

send :: g a -> FFree g a
send fa = FImpure fa FPure
#+END_SRC

** Хелперы
:PROPERTIES:
:REVEAL_EXTRA_ATTR: data-auto-animate
:END:
#+ATTR_REVEAL: :data_id freer :code_attribs data-line-numbers=1-18|11-18
#+BEGIN_SRC haskell
data Bot a where
  PullUpdates :: Bot [Message]
  SendMessage :: Message -> Bot ()
  Translate   :: Text -> Bot (Maybe Text)

type BotM = FFree Bot

send :: g a -> FFree g a
send fa = FImpure fa FPure

pullUpdates :: BotM [Message]
pullUpdates = send PullUpdates

sendMessage :: Message -> BotM [Message]
sendMessage = send . SendMessage

translate :: Text -> BotM (Maybe Text)
translate :: send . Translate
#+END_SRC

** Бизнес логика
#+ATTR_REVEAL: :data_id freer :code_attribs data-line-numbers
#+BEGIN_SRC haskell
proceed :: Bot ()
proceed = pullUpdates
  >>= traverse_ (translateMsg >>= sendMessage)
  where
    translateMsg :: Message -> Bot Message
    translateMsg (Msg ci t) = translate t
      <&> (maybe
             (Msg ci "Cannot translate text")
             (Msg ci))
#+END_SRC

** Интерпретация
#+ATTR_REVEAL: :data_id freer :code_attribs data-line-numbers
#+BEGIN_SRC haskell
data Bot a where
  PullUpdates :: Bot [Message]
  SendMessage :: Message -> Bot ()
  Translate   :: Text -> Bot (Maybe Text)

botIO :: Bot a -> IO a
botIO PullUpdates       = ...
botIO (SendMessage msg) = ...
botIO (Translate t)     = ...
#+END_SRC

** Интерпретация
#+ATTR_REVEAL: :data_id freer :code_attribs data-line-numbers
#+BEGIN_SRC haskell
data FFree g a where
  FPure   :: a -> FFree g a
  FImpure :: g x -> (x -> FFree g a) -> FFree g a

runFree
  :: Monad m
  => (f a -> m a)
  -> FFree f a
  -> m a
runFree phi m = loop m
  where
    loop (FPure x)     = pure x
    loop (FImpure u q) = phi u $ loop q
#+END_SRC

** Интерпретация
#+ATTR_REVEAL: :data_id freer :code_attribs data-line-numbers
#+BEGIN_SRC haskell
runFree
  :: Monad m
  => (f a -> m a)
  -> FFree f a
  -> m a

botIO :: Bot a -> IO a

interpretBotIO :: FFree Bot a -> IO a
interpretBotIO = runFree botIO
#+END_SRC

** Плюсы:
:PROPERTIES:
:REVEAL_EXTRA_ATTR: data-auto-animate
:END:
** Плюсы:
:PROPERTIES:
:REVEAL_EXTRA_ATTR: data-auto-animate
:END:
+ Производительность;
+ Лаконичность;

** Лаконичность
:PROPERTIES:
:REVEAL_EXTRA_ATTR: data-auto-animate
:END:
*Free*
#+ATTR_REVEAL: :data_id freer :code_attribs data-line-numbers
#+BEGIN_SRC haskell
data BotF a
  = PullUpdates ([Message] -> a)
  | SendMessage Message a
  | Translate Text ((Maybe Text) -> a)
#+END_SRC

** Лаконичность
:PROPERTIES:
:REVEAL_EXTRA_ATTR: data-auto-animate
:END:
*Free*
#+ATTR_REVEAL: :data_id freer :code_attribs data-line-numbers
#+BEGIN_SRC haskell
data BotF a
  = PullUpdates ([Message] -> a)
  | SendMessage Message a
  | Translate Text ((Maybe Text) -> a)
#+END_SRC

*Freer*
#+ATTR_REVEAL: :data_id freer :code_attribs data-line-numbers
#+BEGIN_SRC haskell
data Bot a where
  PullUpdates :: Bot [Message]
  SendMessage :: Message -> Bot ()
  Translate   :: Text -> Bot (Maybe Text)
#+END_SRC

** Лаконичность
:PROPERTIES:
:REVEAL_EXTRA_ATTR: data-auto-animate
:END:
*Free*
#+ATTR_REVEAL: :data_id freer :code_attribs data-line-numbers
#+BEGIN_SRC haskell
pullUpdates :: Bot [Message]
pullUpdates = liftF $ PullUpdates id

sendMessage :: Message -> Bot ()
sendMessage msg = liftF $ SendMessage msg ()

translate :: Text -> Bot (Maybe Text)
translate t = liftF $ Translate t id
#+END_SRC

** Лаконичность
:PROPERTIES:
:REVEAL_EXTRA_ATTR: data-auto-animate
:END:
*Free*
#+ATTR_REVEAL: :data_id freer :code_attribs data-line-numbers
#+BEGIN_SRC haskell
pullUpdates :: Bot [Message]
pullUpdates = liftF $ PullUpdates id

sendMessage :: Message -> Bot ()
sendMessage msg = liftF $ SendMessage msg ()

translate :: Text -> Bot (Maybe Text)
translate t = liftF $ Translate t id
#+END_SRC

*Freer*
#+BEGIN_SRC haskell
pullUpdates :: BotM [Message]
pullUpdates = send PullUpdates

sendMessage :: Message -> BotM [Message]
sendMessage = send . SendMessage

translate :: Text -> BotM (Maybe Text)
translate :: send . Translate
#+END_SRC

* Effects systems
** Freer monad
:PROPERTIES:
:REVEAL_EXTRA_ATTR: data-auto-animate
:END:
#+ATTR_REVEAL: :data_id freer :code_attribs data-line-numbers
#+BEGIN_SRC haskell
data FFree g a where
  FPure   :: a -> FFree g a
  FImpure :: g x -> (x -> FFree g a) -> FFree g a
#+END_SRC

** Алгебра бизнес логики
:PROPERTIES:
:REVEAL_EXTRA_ATTR: data-auto-animate
:END:
#+ATTR_REVEAL: :data_id freer :code_attribs data-line-numbers
#+BEGIN_SRC haskell
data Bot a where
  PullUpdates :: Bot [Message]
  SendMessage :: Message -> Bot ()
  Translate   :: Text -> Bot (Maybe Text)
#+END_SRC

** Алгебра бизнес логики
:PROPERTIES:
:REVEAL_EXTRA_ATTR: data-auto-animate
:END:
#+ATTR_REVEAL: :data_id freer :code_attribs data-line-numbers
#+BEGIN_SRC haskell
data Messenger a where
  PullUpdates :: Messenger [Message]
  SendMessage :: Message -> Messenger ()

data Translator a where
  Translate   :: Text -> Translator (Maybe Text)
#+END_SRC

** Тип суммы
#+ATTR_REVEAL: :data_id freer :code_attribs data-line-numbers
#+BEGIN_SRC haskell
data Sum f g a
  = L (f a)
  | R (g a)
#+END_SRC

** Варианты расширения
+ Постороение дерева (Hierarchical Free Monads);
+ Расширяемый спиcок эффектов (Extensible effects);

** Union type
:PROPERTIES:
:REVEAL_EXTRA_ATTR: data-auto-animate
:END:
#+ATTR_REVEAL: :data_id freer :code_attribs data-line-numbers
#+BEGIN_SRC haskell
data Union (sig :: [Type -> Type]) r
#+END_SRC

** Union type
:PROPERTIES:
:REVEAL_EXTRA_ATTR: data-auto-animate
:END:
#+ATTR_REVEAL: :data_id freer :code_attribs data-line-numbers
#+BEGIN_SRC haskell
data Union (sig :: [Type -> Type]) r

Union '[Messenger, Translator]
#+END_SRC

** Member class
:PROPERTIES:
:REVEAL_EXTRA_ATTR: data-auto-animate
:END:
#+ATTR_REVEAL: :data_id freer :code_attribs data-line-numbers=1-7|3-5|7
#+BEGIN_SRC haskell
data Union (sig :: [Type -> Type]) r

class Member f r where
  inj :: f a -> Union r a
  prj :: Union r a -> Maybe (f a)

decomp :: Union (f ': r) a -> Either (Union r a) (f a)
#+END_SRC

** Freer monad
:PROPERTIES:
:REVEAL_EXTRA_ATTR: data-auto-animate
:END:
#+ATTR_REVEAL: :data_id freer :code_attribs data-line-numbers
#+BEGIN_SRC haskell
data FFree f a where
  FPure   :: a -> FFree f a
  FImpure :: f x -> (x -> FFree f a) -> FFree f a
#+END_SRC

** Freer monad
:PROPERTIES:
:REVEAL_EXTRA_ATTR: data-auto-animate
:END:
#+ATTR_REVEAL: :data_id freer :code_attribs data-line-numbers=1-8|5-8
#+BEGIN_SRC haskell
data FFree f a where
  FPure   :: a -> FFree f a
  FImpure :: f x -> (x -> FFree f a) -> FFree f a


data FEFree r a where
  FEPure   :: a -> FEFree r a
  FEImpure :: Union r x -> (x -> FEFree f a) -> FEFree r a
#+END_SRC

** Lift helper
:PROPERTIES:
:REVEAL_EXTRA_ATTR: data-auto-animate
:END:
#+ATTR_REVEAL: :data_id freer :code_attribs data-line-numbers=1-12|7-12|11-12
#+BEGIN_SRC haskell
data Union (sig :: [Type -> Type]) r

class Member f r where
  inj :: f a -> Union r a
  prj :: Union r a -> Maybe (f a)

data Eff r a where
  Pure   :: a -> Eff r a
  Impure :: Union r x -> (x -> Eff r a) -> Eff r a

send :: Member f r => f a -> Eff r a
send fa = Impure (inj fa) Pure
#+END_SRC

** Хeлперы
:PROPERTIES:
:REVEAL_EXTRA_ATTR: data-auto-animate
:END:
#+ATTR_REVEAL: :data_id freer :code_attribs data-line-numbers
#+BEGIN_SRC haskell
send :: Member f r => f a -> Eff r a
send fa = Impure (inj fa) Pure

pullUpdates
  :: Member Messager r
  => Eff r [Message]
pullUpdates = send PullUpdates

sendMessage
  :: Member Messager r
  => Message
  -> Eff r [Message]
sendMessage = send . SendMessage

translate
  :: Member Transaltor r
  => Text
  -> Eff r (Maybe Text)
translate = send . Translate
#+END_SRC

** Бизнес логика
#+ATTR_REVEAL: :data_id freer :code_attribs data-line-numbers
#+BEGIN_SRC haskell
proceed :: (Member Messenger r, Member Translator r) => Eff r ()
proceed = pullUpdates
  >>= traverse_ (translateMsg >>= sendMessage)
  where
    translateMsg :: Member Messenger r => Message -> Eff r Message
    translateMsg (Msg ci t) = translate t
      <&> (maybe
             (Msg ci "Cannot translate text")
             (Msg ci))
#+END_SRC

** Сравнение с TF
*Effect system*
#+ATTR_REVEAL: :data_id freer :code_attribs data-line-numbers
#+BEGIN_SRC haskell
proceed :: (Member Messenger r, Member Translator r) => Eff r ()
#+END_SRC

*Tagless final*
#+ATTR_REVEAL: :data_id freer :code_attribs data-line-numbers
#+BEGIN_SRC haskell
proceed :: (MonadMessenger m, MonadTranslator m) => m ()
#+END_SRC

** Бизнес логика
:PROPERTIES:
:REVEAL_EXTRA_ATTR: data-auto-animate
:END:
#+ATTR_REVEAL: :data_id freer :code_attribs data-line-numbers=1-9|2-3
#+BEGIN_SRC haskell
proceed :: (Member Messenger r, Member Translator r) => Eff r ()
proceed = pullUpdates
  >>= traverse_ (translateMsg >=> sendMessage)
  where
    translateMsg :: Member Messenger r => Message -> Eff r Message
    translateMsg (Msg ci t) = translate t
      <&> (maybe
             (Msg ci "Cannot translate text")
             (Msg ci))
#+END_SRC

** Бизнес логика
:PROPERTIES:
:REVEAL_EXTRA_ATTR: data-auto-animate
:END:
#+ATTR_REVEAL: :data_id freer :code_attribs data-line-numbers=2-4
#+BEGIN_SRC haskell
proceed :: (Member Messenger r, Member Translator r) => Eff r ()
proceed = do
  updates <- pullUpdates
  traverse_ (translateMsg >=> sendMessage) updates
  where
    translateMsg :: Member Messenger r => Message -> Eff r Message
    translateMsg (Msg ci t) = translate t
      <&> (maybe
             (Msg ci "Cannot translate text")
             (Msg ci))
#+END_SRC

** Бизнес логика
:PROPERTIES:
:REVEAL_EXTRA_ATTR: data-auto-animate
:END:
#+ATTR_REVEAL: :data_id freer :code_attribs data-line-numbers=2-5,8-10
#+BEGIN_SRC haskell
proceed :: (Member Messenger r, Member Translator r) => Eff r ()
proceed = do
  updates <- pullUpdates
  runReader ctx
   $ traverse_ (translateMsg >=> sendMessage) updates
  where
    translateMsg
      :: ( Member Messenger r
         , Member (Reader ctx) r )
      => Message -> Eff r Message
    translateMsg (Msg ci t) = translate t
      <&> (maybe
             (Msg ci "Cannot translate text")
             (Msg ci))
#+END_SRC

** Бизнес логика
:PROPERTIES:
:REVEAL_EXTRA_ATTR: data-auto-animate
:END:
#+ATTR_REVEAL: :data_id freer :code_attribs data-line-numbers=2-6,9-12
#+BEGIN_SRC haskell
proceed :: (Member Messenger r, Member Translator r) => Eff r ()
proceed = do
  updates <- pullUpdates
  runReader ctx
   $ runRandom
   $ traverse_ (translateMsg >=> sendMessage) updates
  where
    translateMsg
      :: ( Member Messenger r
         , Member Random r
         , Member (Reader ctx) r )
      => Message -> Eff r Message
    translateMsg (Msg ci t) = translate t
      <&> (maybe
             (Msg ci "Cannot translate text")
             (Msg ci))
#+END_SRC

** Интерпретация
#+ATTR_REVEAL: :data_id freer :code_attribs data-line-numbers
#+BEGIN_SRC haskell
decomp :: Union (f ': r) a -> Either (Union r a) (f a)

interpret
  :: (eff a -> Eff effs)
  -> Eff (eff ': effs)
  -> Eff effs
interpret _   (Pure a)     = Pure a
interpret phi (Impure u f) = case decomp u of
  Right fa -> phi fa
  Left u'  -> Impure u' $ \x -> interpret phi (f x)
#+END_SRC

** Интерпретация
#+ATTR_REVEAL: :data_id freer :code_attribs data-line-numbers=1-8|1-4|6|8
#+BEGIN_SRC haskell
reinterpret
  :: (f a -> Eff (g ': effs))
  -> Eff (f ': effs)
  -> Eff (g ': effs)

runM :: Monad m => Eff '[m] a -> m a

run :: Eff '[] a -> a
#+END_SRC

** Effects system libraries
+ freer-simple
+ polysemy
+ fused-effects
+ ...

** Плюсы & минусы
** Плюсы
:PROPERTIES:
:REVEAL_EXTRA_ATTR: data-auto-animate
:END:
** Плюсы
:PROPERTIES:
:REVEAL_EXTRA_ATTR: data-auto-animate
:END:
+ Высокая выразительноть:
  + Реинтерпретация эффектов;
  + Добавление эффектов на лету;
  + Простая подмена интерпретаторов;
  + Поддержка эффектов высшего порядка;
+ Производительность;
** Минусы
:PROPERTIES:
:REVEAL_EXTRA_ATTR: data-auto-animate
:END:
** Минусы
:PROPERTIES:
:REVEAL_EXTRA_ATTR: data-auto-animate
:END:
+ Производительность;
+ Высокий порог входа;

* Спасибо за внимание!
* Questions?

* Appendix

* Type class
:PROPERTIES:
:REVEAL_EXTRA_ATTR: data-auto-animate
:END:
** Type class
:PROPERTIES:
:REVEAL_EXTRA_ATTR: data-auto-animate
:END:
 *Type class* ≈ *Interface*

** Type class
:PROPERTIES:
:REVEAL_EXTRA_ATTR: data-auto-animate
:END:
 *Type class* ≈ *Interface*

#+ATTR_REVEAL: :data_id type-class :code_attribs data-trim data-line-numbers='1-3|1|2|3'
#+BEGIN_SRC haskell
data Bool
  = True
  | False
#+END_SRC

** Type class
:PROPERTIES:
:REVEAL_EXTRA_ATTR: data-auto-animate
:END:
 *Type class* ≈ *Interface*

#+ATTR_REVEAL: :data_id type-class :code_attribs data-trim data-line-numbers='1-6|5-6|5|6'
#+BEGIN_SRC haskell
data Bool
  = True
  | False

class Show a where
  show :: a -> String
#+END_SRC

** Type class
:PROPERTIES:
:REVEAL_EXTRA_ATTR: data-auto-animate
:END:
 *Type class* ≈ *Interface*

#+ATTR_REVEAL: :data_id type-class :code_attribs data-trim data-line-numbers='1-10|8-10|8|9-10'
#+BEGIN_SRC haskell
data Bool
  = True
  | False

class Show a where
  show :: a -> String

instance Show Bool where
  show True  = "True"
  show False = "False"
#+END_SRC

** Функции высшего порядка
#+ATTR_REVEAL: :data_id tf
#+BEGIN_SRC haskell
map :: (a -> b) -> [a] -> [b]
#+END_SRC

** Типы высшего порядка
:PROPERTIES:
:REVEAL_EXTRA_ATTR: data-auto-animate
:END:

#+ATTR_REVEAL: :data_id type-class2 :code_attribs
#+BEGIN_SRC haskell
class Functor f where
  fmap :: (a -> b) -> f a -> f b
#+END_SRC

** Типы высшего порядка
:PROPERTIES:
:REVEAL_EXTRA_ATTR: data-auto-animate
:END:

#+ATTR_REVEAL: :data_id type-class2 :code_attribs data-line-numbers='1-2|1'
#+BEGIN_SRC haskell
class Functor (f :: Type -> Type) where
  fmap :: (a -> b) -> f a -> f b
#+END_SRC

** Типы высшего порядка
:PROPERTIES:
:REVEAL_EXTRA_ATTR: data-auto-animate
:END:
#+ATTR_REVEAL: :data_id type-class2 :code_attribs data-line-numbers='1-6|4-6'
#+BEGIN_SRC haskell
class Functor f where
  fmap :: (a -> b) -> f a -> f b

instance Functor Maybe where
  fmap f (Just a) = Just $ f a
  fmap _ Nothing  = Nothing
#+END_SRC
